...
import { transferInternalSafe } from "../services/database/tables/clients/withdraw-transfer";
import { sendSimpleEmbed, transactionWentWrong } from "../Helpers/simplified_embed_builder";
...
describe("depositCommand", () => {
  it("moves money from wallet to bank and sends success when transaction succeeds", async () => {
    ...
    expect(transferInternalSafeMock).toHaveBeenCalledWith("user-1", "guild-1", 250, "wallet", "bank");
    expect(transactionWentWrongMock).not.toHaveBeenCalled();
    expect(sendSimpleEmbedMock).toHaveBeenCalledWith(interaction, expect.objectContaining({
...
describe("withdrawCommand", () => {
  it("moves money from bank to wallet and sends success when transaction succeeds", async () => {
    ...
    expect(transferInternalSafeMock).toHaveBeenCalledWith("user-1", "guild-1", 400, "bank", "wallet");
    expect(transactionWentWrongMock).not.toHaveBeenCalled();
    expect(sendSimpleEmbedMock).toHaveBeenCalledWith(interaction, expect.objectContaining({
