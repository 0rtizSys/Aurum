...
import { check_cooldown, set_cooldown } from "../../../services/database/tables/cooldowns/cd_manager";
...
export const work_command: Command = {
    ...
    async execute(interaction:ChatInputCommandInteraction){
        ...
        const cd_time = await get_cd_time(guild_id);
        const symbol = await get_eco_symbol(guild_id);
        const cd = await check_cooldown(guild_id, user_id)
        ...
        try{
            ...
            await set_cooldown(guild_id, user_id, cd_time*1000);
